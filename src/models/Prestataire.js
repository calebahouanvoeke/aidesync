// ============================================================
// MODÈLE PRESTATAIRE
// Gestion des professionnels utilisateurs de la plateforme
// ============================================================
...
class Prestataire {
  ...
  static async findById(id) {
    try {
      const result = await query(
        'SELECT * FROM prestataires WHERE id = $1',
        [id]
      );
  ...
  /**
   * Trouver un prestataire par email
   */
  ...
  static async findAll() {
    try {
      const result = await query(
        'SELECT id, email, nom, prenom, telephone, siret, ville, statut_actif, date_creation FROM prestataires ORDER BY date_creation DESC'
      );
      return result.rows;
    } catch (error) {
  ...
  static async create(data) {
    ...
    try {
      ...
      // Insérer le nouveau prestataire
      const result = await query(
   ...
   */
  static async updatePassword(id, ancienMotDePasse, nouveauMotDePasse) {
    try {
      // Récupérer le prestataire
      const prestataire = await this.findById(id);
      if (!prestataire) {
        throw new Error('Prestataire non trouvé');
      }

      // Vérifier l'ancien mot de passe
      const isValid = await bcrypt.compare(ancienMotDePasse, prestataire.mot_de_passe);
      if (!isValid) {
      ...
      const rounds = parseInt(process.env.BCRYPT_ROUNDS) || 10;
      const hashedPassword = await bcrypt.hash(nouveauMotDePasse, rounds);
    ...
    try {
      ...
      const interventionsResult = await query(
        ...
        WHERE prestataire_id = $1 
          AND date_intervention BETWEEN $2 AND $3`,

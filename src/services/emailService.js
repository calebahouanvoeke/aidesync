...
class EmailService {
      ...
      const html = `<!DOCTYPE html><html><head><style>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <center><a href="${process.env.APP_URL || 'https://aidesync.onrender.com'}/client/${client.lien_acces_securise}" class="button">Acc&eacute;der &agrave; mon espace</a></center>
      ...
      const html = `<!DOCTYPE html><html><head><style>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <center><a href="${process.env.APP_URL || 'https://aidesync.onrender.com'}/client/${client.lien_acces_securise}/factures" class="button">Voir mes factures</a></center>
            <p>Cordialement,</p>
          </div>
        </div>
      </body></html>`;
  ...
  // ─────────────────────────────────────────────────────────────
  // Prestataire notifié qu'un client déclare un paiement
  // ─────────────────────────────────────────────────────────────
      ...
      const html = `<!DOCTYPE html><html><head><style>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <center><a href="${process.env.APP_URL || 'https://aidesync.onrender.com'}/dashboard" class="button">Voir et valider le paiement</a></center>
          </div>
        </div>
      </body></html>`;

      let attachments = [];
      if (facture.preuve_paiement_path) {
        const fs = require('fs');
        const path = require('path');
        const fullPath = path.join(__dirname, '../../public', facture.preuve_paiement_path);
        const ext = path.extname(facture.preuve_paiement_path);
        const content = fs.readFileSync(fullPath).toString('base64');
        attachments = [{
          name: `preuve-paiement-${facture.numero_facture}${ext}`,
          content
        }];
      }
  ...
  // ─────────────────────────────────────────────────────────────
  // Client notifié que son paiement est invalidé
  // ─────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────
  // Nouvelles factures du mois (generateMonthly)
  // ─────────────────────────────────────────────────────────────
  static async sendNouvellesFacturesEmail(client, factures, prestataire) {
    try {
      ...
      const appUrl     = process.env.APP_URL || 'https://aidesync.onrender.com';
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4ff;padding:40px 20px;">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    ...
    <tr><td style="background:#fff;padding:36px 40px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="border:1.5px solid #e2e8f0;border-radius:12px;overflow:hidden;margin-bottom:24px;">
        <thead>
          <tr style="background:#f8fafc;">
            <th style="padding:10px 16px;text-align:left;font-size:11px;text-transform:uppercase;color:#94a3b8;font-weight:700;border-bottom:1px solid #e2e8f0;">N&deg; Facture</th>
            <th style="padding:10px 16px;text-align:left;font-size:11px;text-transform:uppercase;color:#94a3b8;font-weight:700;border-bottom:1px solid #e2e8f0;">Montant TTC</th>
            <th style="padding:10px 16px;text-align:left;font-size:11px;text-transform:uppercase;color:#94a3b8;font-weight:700;border-bottom:1px solid #e2e8f0;">R&eacute;gler</th>
          </tr>
        </thead>
      ...
      <div style="background:#f0f4ff;border-radius:12px;padding:18px 22px;margin-bottom:24px;">
        <p style="font-size:13px;font-weight:700;color:#1e293b;margin:0 0 6px;">&#8505;&#65039;&nbsp; Modes de paiement accept&eacute;s</p>
  ...
  // ─────────────────────────────────────────────────────────────
  // Confirmation paiement validé par le prestataire
  // ─────────────────────────────────────────────────────────────
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fff4;padding:40px 20px;">
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    <tr><td style="background:linear-gradient(135deg,#11998e,#38ef7d);border-radius:16px 16px 0 0;padding:40px 40px 32px;text-align:center;">
      <div style="width:64px;height:64px;background:rgba(255,255,255,.25);border-radius:50%;margin:0 auto 18px;text-align:center;line-height:64px;font-size:30px;">&#10003;</div>
      <h1 style="color:#fff;font-size:24px;font-weight:800;margin:0 0 8px;">Paiement confirm&eacute; !</h1>
      <p style="color:rgba(255,255,255,.85);font-size:14px;margin:0;">Votre r&egrave;glement a bien &eacute;t&eacute; re&ccedil;u et valid&eacute;</p>
    </td></tr>
    <tr><td style="background:#fff;padding:36px 40px;">
      <p style="font-size:16px;color:#1e293b;font-weight:700;margin:0 0 6px;">Bonjour ${client.prenom},</p>
      <p style="font-size:14px;color:#64748b;line-height:1.7;margin:0 0 24px;">
      ...
      <div style="background:#f0fff4;border:1.5px solid #bbf7d0;border-radius:14px;padding:24px 28px;margin-bottom:24px;">
        <div style="text-align:right;margin-bottom:14px;">
          <span style="background:#dcfce7;color:#16a34a;font-size:12px;font-weight:700;padding:4px 14px;border-radius:20px;">&#10003; Pay&eacute;e</span>
        </div>
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="padding:4px 0;vertical-align:top;">
              <span style="font-size:11px;color:#94a3b8;text-transform:uppercase;display:block;margin-bottom:2px;">N&deg; Facture</span>
            ...
            <td style="padding:4px 0;text-align:right;vertical-align:top;">
              <span style="font-size:11px;color:#94a3b8;text-transform:uppercase;display:block;margin-bottom:2px;">Montant r&eacute;gl&eacute;</span>
  ...
  static async testEmailConfig() {
    try {
      const apiInstance = this.getApiInstance();
      console.log('✅ Configuration Brevo API OK');
      return true;
    } catch (error) {
      console.error('❌ Erreur configuration Brevo API:', error);
      return false;
    }
  }
}
